# Draft: Custom Actions

Use the Custom Actions beta to build integrations directly into [Frame.io](http://frame.io/) as programmable UI components. This enables workflows that can be triggered by users within the app, leveraging the same underlying events routing as webhooks. You can create user-triggered single or multi-step forms that come back to Frame.io as another form or a basic response. And when a user clicks a Custom Action on an Asset, [Frame.io](http://frame.io/) sends a payload to a URL you provide. The receiving application responds with an HTTP status code to acknowledge receipt, or responds with a custom callback that can render additional UI in Frame.io.
...
**Checkbox:** A simple checkbox with no additional parameters.
...
**Link:** A simple link with no additional parameters.
